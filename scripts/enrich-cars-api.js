// Script para enriquecer os dados dos carros via API local
...
// Primeiro buscar os carros existentes para pegar os IDs
async function fetchCars() {
	return new Promise((resolve, reject) => {
		http.get(`${BASE_URL}/api/cars`, (res) => {
			let data = ''
...
async function main() {
	try {
		console.log('Fetching existing cars...')
		const { cars } = await fetchCars()
		console.log(`Found ${cars.length} cars\n`)

		for (const enrichData of enrichedCarsData) {
			const existingCar = cars.find((c) => c.licensePlate === enrichData.licensePlate)
			if (!existingCar) {
				console.log(`Car not found: ${enrichData.licensePlate}`)
				continue
			}

			console.log(`Updating: ${existingCar.licensePlate} (${existingCar._id})...`)

			const result = await updateCar(existingCar._id, enrichData)

			if (result.status === 200) {
				console.log(`  SUCCESS: ${enrichData.name.substring(0, 50)}...`)
			} else {
